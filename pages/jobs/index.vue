        ...
        <ul class="mx-5">
            <li class="md:p-7 p-4 md:my-4 my-12 bg-white mb-3 mx-auto rounded-md shadow-lg" v-for="(job, index) in getJobs">
                <div class="flex flex-wrap">
                    ...
                    <nuxt-link :to="{path: `/jobs/${job._id}/edit`}">Edit</nuxt-link>
                    <button class="primary light-grayish-cyan-bg rounded-sm text-sm" @click="jobs.confirmDelete(job._id)">Delete</button>
                </div>
            </li>
        </ul>
      </div>
